...
function loadBlocks(){
    ...
    /* request = $.ajax({
      url: document.location.origin + "/pylos/github",
      type: "post"
    }); */
...
function feedData(data){
    ...
    if (!data.length){
      ...
      page = -1;
      d3.select(".loading").html("Aww shoot! No block found for " + keywords.join(", ") +' Sorry! :(. Try a different keyword or <a href="./hydra"> show all blocks</a>...');
      return [];
