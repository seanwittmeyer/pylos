	...
	<div class="container top-nospace">
		<div class="row">
			...
			<div class="col-lg-12" id="graphcontainer">
				...
				<?php if (!isset($payload['hostdiagram'])) echo '<p><a data-toggle="modal" data-target="#pageeditor">Click here to set up your diagram.</a><p>'; ?>
				<div id="graph-info"><button id="save">Save Image</button></div>
				<div id="svgdataurl"></div>
				<div id="pngdataurl"></div>
				<canvas style="display:none"></canvas>
				<script>
					canvasheight_var = $("#graphcontainer").width()
					$('canvas').width(canvasheight_var);
					$('canvas').height(canvasheight_var*.7);

				</script>
			</div>
			<?php if (isset($payload['hostdiagram'])) : ?>
				...
				(function() {
					...
					d3.json("/api/visualization/conversation/<?php echo $payload['hostdiagram']; ?>", function(X, Y) {
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						<label for="payload[payload][hostdiagram]" class="">Diagram Host</label>
						<p>Thi will control which taxonomy/category that will be at the center of the diagram on this page. Select 1 from the list below and try it out!</p>
					</div>
					<div class="form-group">
						<label class="col-sm-2 control-label" style="padding-top: 16px;">Host</label>
						<div class="col-sm-10">
							<select name="payload[payload][hostdiagram]" class="selectpicker btn-sm" data-width="100%" data-live-search="true" data-size="5">
							<?php // List taxonomy
								$list_tax = $this->shared->list_bytype('taxonomy');
								if ($list_tax === false) { echo '<option disabled>No taxonomy to display.</option>'; } else {
								foreach ($list_tax as $a) { $selected = ($a['id'] == $payload['hostdiagram']) ? ' selected' : ''; echo '<option value="'.$a['id'].'"'.$selected.'>'.$a['title']."</option>\n"; }} ?> 
							</select>
						</div>
					</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									...
									<?php // List taxonomy
										...
										if ($list_tax === false) { echo '<option disabled>No taxonomy to display.</option>'; } else {
										foreach ($list_tax as $a) { $selected = (in_array($a['id'],$relationships)) ? ' selected' : ''; echo '<option value="'.$a['id'].'"'.$selected.'>'.$a['title']."</option>\n"; }} ?> 
									</select>
								</div>
							</div>
						</div>
