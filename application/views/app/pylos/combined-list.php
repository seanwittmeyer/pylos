<!-- Content Area -->
		...
		<div class="row hideonsearch pylos-section-hero" style="min-height: unset;">
			...
			<div class="col-lg-8">
				<h3 style="font-weight: 600;"><?php echo $dependency['title']; ?></h3>
				<h3 style="font-weight: 600;"><?php echo $dependency['excerpt']; ?></h3>
				...
				<ul class="pylos-section-tags" style="padding:0;">
					<a href="#" onclick="$('.smallchildren').show(); return false;"><span class="fa fa-question-circle"></span>Learn More</a>
					<a href="<?php echo $dependency['url']; ?>"><span class="fa fa-download"></span>Get <?php echo $dependency['title']; ?></a>
					<span>or</span>
					<a href="#" onclick="$('#pylos-edit-dependency').show('fast'); return false;"><span class="fa fa-pencil"></span>Help Update this Page</a>
				</ul>
				<hr />
			</div>
			<div class="col-lg-8 col-lg-offset-2">
			...
			<div class="col-lg-8">
				...
				<h3 style="font-weight: 600;/* text-transform: none; */">
					<?php if (strtolower($slug) == 'dependency') { ?>
						<a href="<?php echo site_url('pylos/dependencies/'.$tag); ?>"><?php echo urldecode($tag); ?></a> is a tool.</h3><h3 style="font-weight: 600;/* text-transform: none; */">These are resources that use and possibly require <a href="<?php echo site_url('pylos/dependency/'.$tag); ?>"><?php echo urldecode($tag); ?></a>. <a href="#" onclick="$('#pylos-edit-dependency').show('fast'); return false;">Help us by building out the Pylos record</a> if you are a power user and can share a description and install files for this dependency. 
					<?php } else { ?>These are resources and components for computational design that you can plug into your tool, analysis, or design process already made.<?php } ?>
				</h3>
			</div>
		...
		<div id="pylos-edit-dependency" class="row" style="display: none;">
			<div class="col-sm-12">
				...
				<form id="formeditdependency" class="form-horizontal" enctype="multipart/form-data">
					...
					<input id="pylos-edit-dependency-type" type="hidden" name="payload[type]" value="dependency" />
		...
		<?php if (!is_array($dependency) && $slug===true) { ?>
		<!-- Hero Content Header --> 
		<div class="row hideonsearch pylos-section-hero" style="min-height: unset;">
			<div class="col-lg-2">
				<h1><i class="fa fa-puzzle-piece"></i></h1>
			...
			<div class="col-lg-8">
				...
				<h3 style="font-weight: 600;">
					<?php if (strtolower($slug) == 'dependency') { ?>
						<a href="<?php echo site_url('pylos/dependency/'.$tag); ?>"><?php echo urldecode($tag); ?></a> is a tool.</h3><h3 style="font-weight: 600;">These are blocks that use and possibly require <a href="<?php echo site_url('pylos/dependency/'.$tag); ?>"><?php echo urldecode($tag); ?></a>. <a href="#" onclick="$('#pylos-edit-dependency').show('fast'); return false;">Help us by building out the Pylos record</a> if you are a power user and can share a description and install files for this dependency. 
					<?php } else { ?>These are the resources that use <?php echo urldecode($tag); ?> that you can plug into your tool, analysis, or design process already made.<?php } ?>
				</h3>
			</div>
			<ul class="col-lg-10 col-lg-offset-2 pylos-section-tags">
				<span style="padding-left: 0;">Popular Themes: </span>
		...
		<?php } ?>
		<?php if ($slug == "project") { 
			$record = $this->pylos_model->get_curl2('nexus','projects/'.strtoupper($tag),array('type'=>'project','arg1'=>strtoupper($tag)));
			if (isset($record['returnedResults'])) { 
				$project = (array)$record['results'][0];
			?>
			<!-- Hero Content Header --> 
			<div class="row hideonsearch pylos-section-hero" style="min-height: unset;">
				<div class="col-lg-2">
				...
				<div class="col-lg-8">
					<h3 style="font-weight: 600; text-transform: none;"><?php echo $project['projectName']; ?><br /><?php echo number_format($project['squareFootage']); ?> sf <?php echo strtolower($project['projectType']); ?> <?php echo strtolower($project['loB_EntityCategory']); ?>.</h3>
					<h3 style="font-size: 18px; font-weight: 600; text-transform: none;">
						<?php echo $project['clientName']; ?><br />
						<?php if (isset($project['streetAddress1'])) echo $project['streetAddress1'].', '; echo $project['city']; ?>, <?php echo $project['state']; ?> <?php if (isset($project['zip'])) echo $project['zip']; ?><br />
						<?php echo $project['projectNumber']; ?><br />
					</h3>
				</div>
				<ul class="col-lg-10 col-lg-offset-2 pylos-section-tags">
					<span style="padding-left: 0;">Project links: </span>
					<a href="<?php echo $project['daM_ProjectRecord']; ?>">Open Asset Images</a>
					<a href="<?php echo $project['piM_ProjectRecord']; ?>">Newforma</a>
					<a href="<?php echo $project['loB_ProjectRecord']; ?>">Vision</a>
					<a href="<?php echo $project['projectFolderLocation']; ?>">Project Folder</a>
				</ul>
			</div>
			<!-- / Content Header --> 
			<?php } ?>
