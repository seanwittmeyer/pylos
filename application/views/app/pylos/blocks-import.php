		...
		<div class="row">
			<div class="col-lg-11">
				...
				<form id="formnewblock" class="form-horizontal" enctype="multipart/form-data">
					...
					<input id="pylos-block-thumbnail" type="hidden" name="payload[importthumbnail]" value="<?php if (isset($block['thumbnail'])) echo $block['thumbnail']; ?>" />
					<p>You made something and want to share it, now let's get a good title so people can find your resource.</p>
					<?php /*<pre><?php var_dump($block); ?></pre>*/ ?>
					...
					<div class="form-group">
						...
						<div class="col-md-9"><input type="text" class="form-control" id="pylos_newblock_title" name="payload[title]" value="<?php echo $block['title']; ?>" placeholder="Simple and clear!"></div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-md-9"><textarea type="text" class="pylos-summernote" id="pylos_newblock_description" name="payload[description]" value="<p><?php echo $block['description']; ?></p>" placeholder="Describe this and include how it works and any information people will need to use it..."><p><?php echo $block['description']; ?></p></textarea></div>
					</div>
					...
					<div class="form-group">
						<div class="col-sm-offset-2 col-md-9">
							<div class="checkbox">
								<label>
									<input type="checkbox" id="pylos_newblock_private" checked="checked" name="payload[private]"> Limit access to this resource to the ZGF network
								</label>
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-md-9"><input type="text" class="form-control" name="payload[tags]" id="pylos_newblock_tags" value="<?php foreach ($block['tags'] as $i) { echo $i.',';} ?>" placeholder="analysis, energyplus, etc..."><span class="small">If you don't see the tags you want in the list above, you can add them here. Separate by commas.</span></div>
					</div>
					<div class="form-group">
						<label for="payload[dependencies]" class="col-md-3 control-label">Dependencies</label>
						<div class="col-md-9"><input type="text" class="form-control" name="payload[dependencies]" id="pylos_newblock_dependencies" value="<?php foreach ($block['dependencies'] as $i) { echo $i.',';} ?>" placeholder="diva, honeybee, human ui, etc..."><span class="small">If you don't see the tags you want in the list above, you can add them here. Separate by commas.</span></div>
					</div>
					<div class="form-group">
						<label for="payload[components]" class="col-md-3 control-label">Components</label>
						<div class="col-md-9"><input type="text" class="form-control" name="payload[components]" id="pylos_newblock_components" value="<?php foreach ($block['components'] as $i => $j) { echo $i.' x'.$j.', ';} ?>" placeholder="diva, honeybee, human ui, etc..."><span class="small">These are the components we found in your definition.</span></div>
					</div>
					<div class="form-group">
						<label for="payload[project]" class="col-md-3 control-label">Project Number</label>
						<div class="col-md-9"><input type="text" class="form-control" name="payload[project]" id="pylos_newblock_project" placeholder="23796.udan"><span class="small">Adding a project helps folks learn how certain things were done if they know a project. Leave this blank if the resource was intended to be generic.</span></div>
					</div>

					<!--
					<div class="form-group">
						<label class="col-md-3 control-label" style="padding-top: 16px;">Dependencies</label>
						<div class="col-md-9">
							<select name="payload[dependencies][]" class="selectpicker btn-sm" data-width="100%" data-live-search="true" data-size="5" multiple data-selected-text-format="count > 4">
							<?php // List taxonomy
...
<!-- End Content Area -->
	...
	<script>
		...
		var <?php $file = array_pop($block['images']); foreach ($block['images'] as $i => $j) { ?>pylosUploadUrl<?php echo $i; ?> = '<?php echo site_url($j); ?>',<?php echo "\n			"; } ?>
			pylosUploadInitial = true;
		$('#pylos-block-images').fileinput({
		    ...
		    minFileCount: 1,
		    maxFileCount: 10,
			initialPreview: [<?php foreach ($block['images'] as $i => $j) { ?>pylosUploadUrl<?php echo $i; if (count($block['images']) != $i+1) echo ','; } ?>],
	        initialPreviewAsData: true,
			initialPreviewConfig: [
				<?php foreach ($block['images'] as $i => $j) { ?>{caption: "Image #<?php echo $i+1; ?>", type: 'image', fileType: "<?php echo get_mime_by_extension(".".site_url($j)); ?>", downloadUrl: pylosUploadUrl<?php echo $i; ?>, width: "120px", key: <?php echo $i+1; ?>}<?php if (count($block['images']) != $i+1) echo ','; } ?>
			],
		...
		$('#pylos-block-file').fileinput({
			initialPreview: ["<?php echo site_url($file); ?>"],
	        initialPreviewAsData: true,
	        initialPreviewConfig: [
	            {caption: "Uploaded Zip", type: 'other', fileType: "application/zip", downloadUrl: "<?php echo site_url($file); ?>", size: <?php $size = get_file_info(".".$file, 'size'); echo $size['size']; ?>, width: "120px", key: 1}
	        ],
	        deleteUrl: "/site/file-delete",
			...
			$.ajax({
				...
				statusCode: {
					200: function(data) {
						...
						// <?php echo $newunique; ?> 
						window.location.assign(response.result.url + '?firstrun=true&import=<?php echo $newunique; ?>');
					},
					403: function(data) {
						//var response = JSON.parse(data);
						$('#newblockloading').hide(); 
